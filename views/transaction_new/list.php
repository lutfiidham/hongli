...
function load_data_history(id)
{
	...
	//$("#table_history").load('transaction_new.php?page=list_history&table_id='+id); 
               ...
               Simpan Berhasil
                ...
                <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
                <b>Simpan Gagal !</b>
               Harga Menu masih kosong, Pilih menu terlebih dahulu !
                </div>
 ...
 <form action="<?= $action ?>" method="post" enctype="multipart/form-data" role="form">
                ...
                <section class="content" style="padding-top: 0">
                  <?php
								...
								$get_all_item	= get_all_item($table_id);
                                ?>
              <div class="row">
              <div <?php if($get_all_item == 0){ ?>class="col-md-12" <?php }else{ ?>class="col-md-8"<?php } ?> id="table_menu">
             ...
             <div class="form-group">
              ...
              <div id="table_history">
               ...
               <div class="row">
                ...
                <div class="col-md-4" style="padding-left:0px;" >
                  <div class="form-group">
                    <a style="white-space: pre-wrap" href="transaction_new.php?page=list&table_id=<?= $_GET['table_id']; ?>&mt_id=<?= $r_cat_button['menu_type_id']?>" class="btn btn-block btn_cat_button"> <?= $r_cat_button['menu_type_name'] ?></a>
                  </div>
                </div>
                <?php
                }
               ...
               <!-- batas kategori 
				...
				<p>
                  ...
                  <?php
                    $no2 = 1;
          $query = mysql_query("select * from menus where menu_type_id = '".$row_cat['menu_type_id']."' order by ordered DESC, menu_name asc");
                  ...
                  <?php if(isset($_GET['mt_id']) && $_GET['mt_id'] != $row_cat['menu_type_id']){ ?> style="display:none;"<?php } ?>>
                   ...
                   <a id="dialogModal_ex<?= $row['menu_id']?>" data-variable="<?= $row['menu_id']?>">
                    <input type="hidden" value="<?= $row_cat['menu_type_id']?>" id="hid<?= $row['menu_id']?>">
...
?>
    $('#dialogModal_ex<?= $row_popmodal['menu_id'] ?>').click(function(){
		...
		var type= $('#hid<?= $row_popmodal['menu_id'] ?>').val();
		// if(type != 2){
			// if(type != 3){
