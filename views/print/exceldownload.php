...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('G1', 'Discount(Rp.)')
			->setCellValue('H1', 'Serv. Charge(5%)')
			->setCellValue('I1', 'Tax(10%)')
			->setCellValue('J1', 'Grand Total')
			->setCellValue('K1', 'Bayar')
			->setCellValue('L1', 'Kembali');
...
while ($row = mysql_fetch_array($query)) {
	...
	$kembali = $kembali+($row['transaction_payment']-$grand);
	$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A'.$i, $j)
            ...
            ->setCellValue('E'.$i, 'Rp. '.number_format($row['transaction_total']))
            ->setCellValue('F'.$i, $row['transaction_discount'].'%')
			->setCellValue('G'.$i, 'Rp. '.number_format($row['transaction_discount']/100*$row['transaction_total']))
			->setCellValue('H'.$i, 'Rp. '.number_format($svc))
			->setCellValue('I'.$i, 'Rp. '.number_format($tax))
			->setCellValue('J'.$i, 'Rp. '.number_format($grand))
			->setCellValue('K'.$i, 'Rp. '.number_format($row['transaction_payment']))
			->setCellValue('L'.$i, 'Rp. '.number_format($row['transaction_payment']-$grand));
				$i++;
				$j++;
}
$k = $i++;
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('E'.$k, 'Rp. '.number_format($total))
			->setCellValue('G'.$k, 'Rp. '.number_format($discount))
			->setCellValue('H'.$k, 'Rp. '.number_format($charge))
			->setCellValue('I'.$k, 'Rp. '.number_format($taxtotal))
			->setCellValue('J'.$k, 'Rp. '.number_format($grandtot))
			->setCellValue('K'.$k, 'Rp. '.number_format($bayar))
			->setCellValue('L'.$k, 'Rp. '.number_format($kembali));
